...
### Combining per-tree distributions

`PartitionForestRegressor.predict_proba` merges the breakpoints of all per-tree
distributions into a single `IntervalDistribution`. The merge is performed in
Rust and returned as flat segment arrays, so it scales to large forests and
sample counts. The resulting object supports the full skpro distribution API
(PDF, CDF, quantiles, mean, variance, energy).
