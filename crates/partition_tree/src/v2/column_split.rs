...
impl ColumnSplitSearcher for ContinuousColumnSplitSearcher {
    fn search(
        ...
        let w_xy_parent: f64 =
            sxy_some.iter().map(|&i| weights_xy[i as usize]).sum::<f64>() + wxy_null;
        ...
        let candidate_weights: Vec<f64> = match split_kind {
            SplitKind::XSplit => candidates
                .iter()
                .map(|&i| weights_x[i as usize])
                .collect(),
            SplitKind::YSplit => candidates
                .iter()
                .map(|&i| weights_y[i as usize])
                .collect(),
        };
        let prefix_candidate = cumsum(&candidate_weights);

        // Prefix sums on sxy_some weights
        let sxy_weights: Vec<f64> = sxy_some
            .iter()
