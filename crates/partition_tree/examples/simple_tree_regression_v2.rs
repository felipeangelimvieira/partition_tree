...
fn main() {
    ...
    // Map v1 params → v2
    //   v1: max_iter=12, min_samples_split=2, min_samples_leaf_y=0,
    //       min_samples_leaf_x=0, min_samples_leaf=0, max_depth=6,
    //       min_target_volume=0, min_split_gain=1e-8, expansion=0.0
    let config = TreeBuilderConfig {
        max_leaves: 12 + 1, // max_iter + 1
...
fn print_results(tree: &Tree) {
    ...
    for (i, rec) in tree.split_history.iter().enumerate() {
        // Extract the split threshold from the left child's cell rule for the split column
